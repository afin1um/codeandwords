...
public class TeamsRepository {
    ...
    private void syncLocalTeamsWithServer(int userId, List<Team> serverTeams) {
        executor.execute(() -> {
            try {
                // Собираем ID команд, которые ЕСТЬ на сервере
                ...
                Log.e(TAG, "Ошибка sync кэша: " + e.getMessage(), e);
            }
        });
    }
    ...
    public void createTeam(String teamName, User currentUser,
        ...
        apiService.insertTeamRaw(payload).enqueue(new Callback<List<JsonObject>>() {
            @Override
    ...
    public void createTeamWithFriends(String teamName,
        ...
        apiService.insertTeamRaw(teamPayload).enqueue(new Callback<List<JsonObject>>() {
            @Override
            public void onResponse(Call<List<JsonObject>> call,
                                   Response<List<JsonObject>> response) {
                Log.d(TAG, "insertTeamRaw: code=" + response.code());
    ...
    private void createTeamMembersRemote(int teamId, List<Integer> memberIds,
                                         String challengeType, int targetValue,
                                         DataCallback<Integer> callback) {
        ...
        apiService.insertTeamMembersMinimalRaw(membersPayload).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                Log.d(TAG, "insertTeamMembersMinimalRaw: code=" + response.code());
            ...
            public void onFailure(Call<Void> call, Throwable t) {
                Log.e(TAG, "Ошибка сети insertTeamMembersMinimalRaw: " + t.getMessage(), t);
                mainHandler.post(() -> callback.onError("Ошибка сети при добавлении участников"));
            }
        });
    }

    private void createTeamChallengeRemote(int teamId, List<Integer> memberIds,
                                           String challengeType, int targetValue,
                                           DataCallback<Integer> callback) {
        ...
        apiService.insertTeamChallengeRaw(payload).enqueue(new Callback<List<JsonObject>>() {
            @Override
            public void onResponse(Call<List<JsonObject>> call,
                                   Response<List<JsonObject>> response) {
                Log.d(TAG, "insertTeamChallengeRaw: code=" + response.code());
                ...
                if (response.isSuccessful()
                    ...
                    if (!created.has("id") || created.get("id").isJsonNull()) {
                        Log.e(TAG, "Ответ не содержит id челленджа");
                        mainHandler.post(() -> callback.onError(
                                "Не удалось получить id задания"));
                        return;
                    }
                    ...
                    mainHandler.post(() -> callback.onError(
                            "Не удалось создать задание команды: " + response.code()));
                }
            }

            @Override
            public void onFailure(Call<List<JsonObject>> call, Throwable t) {
                Log.e(TAG, "Ошибка сети insertTeamChallengeRaw: " + t.getMessage(), t);
                mainHandler.post(() -> callback.onError(
                        "Ошибка сети при создании задания команды"));
            }
        });
    }

    private void createTeamChallengeProgressRemote(int teamId, int challengeId,
        ...
        apiService.insertTeamChallengeProgressMinimalRaw(progressPayload)
                .enqueue(new Callback<Void>() {
                    @Override
                    public void onResponse(Call<Void> call, Response<Void> response) {
                        Log.d(TAG, "insertTeamChallengeProgressMinimalRaw: code="
                                + response.code());
                    ...
                    @Override
                    public void onFailure(Call<Void> call, Throwable t) {
                        Log.e(TAG, "Ошибка сети insertTeamChallengeProgressMinimalRaw: "
                                + t.getMessage(), t);
                        mainHandler.post(() -> callback.onError(
                                "Ошибка сети при создании прогресса"));
                    }
                });
    }
    ...
    public void getMyTeams(User currentUser, DataCallback<List<Team>> callback) {
        ...
        executor.execute(() -> {
            try {
                ...
                if (ownedTeams != null) {
                    for (Team team : ownedTeams) addTeamIfNotExists(localTeams, team);
                }
                ...
                if (memberships != null) {
                    for (TeamMember member : memberships) {
                        ...
                        if (team != null) addTeamIfNotExists(localTeams, team);
                    }
                }

                // Показываем кэш моментально (если он есть)
                if (!localTeams.isEmpty()) {
                    mainHandler.post(() -> callback.onSuccess(localTeams));
                }

                // ВСЕГДА идем на сервер для синхронизации (даже если кэш есть)
    ...
    /**
     * ИСПРАВЛЕНО: теперь после получения данных с сервера вызывается
     * syncLocalTeamsWithServer — он удаляет из локального кэша команды,
     * которых больше нет на сервере. Это решает проблему "удалённые команды
     * всё ещё отображаются".
     */
    private void refreshTeamsFromServer(int userId,
                                        List<Team> localTeams,
                                        DataCallback<List<Team>> callback) {
        apiService.getOwnedTeamsRaw("eq." + userId, "id,team_name,owner_id", "id.desc")
                .enqueue(new Callback<List<JsonObject>>() {
                    ...
                    public void onResponse(Call<List<JsonObject>> ownedCall,
                        ...
                        if (ownedResponse.isSuccessful() && ownedResponse.body() != null) {
                            ...
                            // Сервер вернул ошибку — оставляем кэш как есть
                            Log.e(TAG, "Ошибка getOwnedTeamsRaw: " + ownedResponse.code());
                            if (localTeams.isEmpty()) {
                                mainHandler.post(() ->
                        ...
                        // Загружаем команды, в которых пользователь УЧАСТНИК
                        apiService.getMyTeamMembersRaw("eq." + userId, "id,team_id,user_id")
                                .enqueue(new Callback<List<JsonObject>>() {
                                    @Override
                                    public void onResponse(Call<List<JsonObject>> membersCall,
                                        ...
                                        if (!membersResponse.isSuccessful()
                                            ...
                                            // Не смогли получить список участий — синхронизируем тем, что есть
                                            syncLocalTeamsWithServer(userId, result);
                                        ...
                                        for (JsonObject item : membersResponse.body()) {
                                            if (item.has("team_id")
                                                    && !item.get("team_id").isJsonNull()) {
                                                int tid = item.get("team_id").getAsInt();
                                                if (!teamIds.contains(tid)) teamIds.add(tid);
                                            }
                                        }

                                        if (teamIds.isEmpty()) {
                                            // Пользователь не участник ни в каких командах
                                            syncLocalTeamsWithServer(userId, result);
                                            mainHandler.post(() -> callback.onSuccess(result));
                                            return;
                                        }
                                        ...
                                        apiService.getTeamsByIdsRaw(
                                            ...
                                            public void onResponse(Call<List<JsonObject>> c,
                                                ...
                                                // ВАЖНО: синхронизируем локальный кэш с сервером
                                                syncLocalTeamsWithServer(userId, result);
                                                mainHandler.post(() ->
                    ...
                    public void onFailure(Call<List<JsonObject>> ownedCall, Throwable t) {
                        Log.e(TAG, "Ошибка getOwnedTeamsRaw: " + t.getMessage(), t);
                        if (localTeams.isEmpty()) {
                            mainHandler.post(() ->
                                    callback.onError("Не удалось загрузить команды"));
                        }
    ...
    private void refreshTeamChallengeFromServer(int teamId,
        ...
        Log.d(TAG, "Загружаем челлендж с сервера, teamId=" + teamId);
        ...
        fastApiService.getTeamChallengeRaw(
            ...
            public void onResponse(Call<List<JsonObject>> call,
                ...
                Log.d(TAG, "getTeamChallenge ответ: code=" + response.code());
                ...
                if (response.isSuccessful()
                    ...
                    Log.w(TAG, "Челлендж не найден для teamId=" + teamId);
                    if (!hasCachedData) {
                        mainHandler.post(() ->
    ...
    private void silentRefreshTeamChallenge(int teamId) {
        ...
        fastApiService.getTeamChallengeRaw(
                ...
                if (response.isSuccessful()
                    ...
                    executor.execute(() -> {
                        try {
                            ...
                            Log.d(TAG, "Кэш челленджа обновлён, teamId=" + teamId);
                        } catch (Exception e) {
            ...
            public void onFailure(Call<List<JsonObject>> call, Throwable t) {
                isFetchingChallenge = false;
                Log.w(TAG, "Фоновое обновление не удалось: " + t.getMessage());
            }
        });
    }

    // ===== ПРОГРЕСС КОМАНДЫ: ЗАГРУЗКА (cache-first) =====
        ...
        executor.execute(() -> {
            try {
                ...
                if (cached != null && !cached.isEmpty()) {
                    mainHandler.post(() -> callback.onSuccess(cached));
                    silentRefreshTeamProgress(challengeId);
                } else {
                    mainHandler.post(() ->
                            refreshTeamProgressFromServer(challengeId, false, callback));
                }
    ...
    private void silentRefreshTeamProgress(int challengeId) {
        apiService.getTeamProgressRaw(
            ...
            public void onResponse(Call<List<JsonObject>> call,
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    for (JsonObject item : response.body()) {
                        fresh.add(parseTeamProgress(item));
                    }
                    executor.execute(() -> {
                        try {
            ...
            @Override
            public void onFailure(Call<List<JsonObject>> call, Throwable t) {
                Log.w(TAG, "Фоновое обновление прогресса не удалось: " + t.getMessage());
            }
        });
    }

    private void refreshTeamProgressFromServer(int challengeId,
                                               boolean hasCachedData,
                                               DataCallback<List<TeamChallengeProgress>> callback) {
        Log.d(TAG, "Загружаем прогресс с сервера, challengeId=" + challengeId);
        ...
        apiService.getTeamProgressRaw(
            ...
            public void onResponse(Call<List<JsonObject>> call,
                                   Response<List<JsonObject>> response) {
                Log.d(TAG, "getTeamProgress ответ: code=" + response.code());

                if (response.isSuccessful() && response.body() != null) {
                    List<TeamChallengeProgress> result = new ArrayList<>();
                    for (JsonObject item : response.body()) {
                        result.add(parseTeamProgress(item));
                    }

                    executor.execute(() -> {
                        try {
                            teamChallengeProgressDao.deleteByChallengeId(challengeId);
                            for (TeamChallengeProgress p : result) {
                                teamChallengeProgressDao.insert(p);
                            }
                        } catch (Exception e) {
                            Log.e(TAG, "Ошибка кэширования прогресса: " + e.getMessage(), e);
                        }
                    });
            ...
            @Override
            public void onFailure(Call<List<JsonObject>> call, Throwable t) {
                Log.e(TAG, "Ошибка сети getTeamProgress: " + t.getMessage(), t);
                if (!hasCachedData) {
                    mainHandler.post(() ->
                            callback.onError("Ошибка сети при загрузке прогресса"));
                }
            }
        });
    }
    ...
    public void updateTeamChallengeProgressAfterLesson(int userId, int earnedXp) {
        ...
        apiService.getMyActiveTeamProgressRaw("eq." + userId, "eq.false", select)
                .enqueue(new Callback<List<JsonObject>>() {
                    @Override
                    public void onResponse(Call<List<JsonObject>> call,
                                           Response<List<JsonObject>> response) {
                        ...
                        for (JsonObject progressJson : response.body()) {
                            try {
                                ...
                                Log.e(TAG, "Ошибка обработки прогресса: "
                                        + e.getMessage(), e);
                            }
                        }
                    }
                    ...
                    @Override
                    public void onFailure(Call<List<JsonObject>> call, Throwable t) {
                        Log.e(TAG, "Ошибка сети updateTeamChallengeProgress: "
                                + t.getMessage(), t);
                    }
                });
    }
    ...
    private void processSingleTeamProgress(JsonObject progressJson, int earnedXp) {
        ...
        if (!progressJson.has("is_completed")
                || progressJson.get("is_completed").isJsonNull()) return;
        if (!progressJson.has("challenge")
                || !progressJson.get("challenge").isJsonObject()) return;
        ...
        if (!challengeJson.has("condition_type")
                || challengeJson.get("condition_type").isJsonNull()) return;
        if (!challengeJson.has("target_value")
                || challengeJson.get("target_value").isJsonNull()) return;
        ...
        if (newProgress >= targetValue) {
            ...
            apiService.updateTeamChallengeProgressRaw("eq." + progressId, payload)
                    .enqueue(new Callback<Void>() {
                        @Override
                        public void onResponse(Call<Void> call, Response<Void> response) {
                            if (!response.isSuccessful()) {
                                Log.e(TAG, "Не удалось обновить прогресс: "
                                        + response.code());
                            }
                        }
    ...
    private void finishTeamChallengeProgress(int progressId, int challengeId,
        ...
        apiService.getTeamProgressByIdRaw(
                "eq." + progressId,
            ...
            public void onResponse(Call<List<JsonObject>> checkCall,
                ...
                if (!checkResponse.isSuccessful() || checkResponse.body() == null
                        || checkResponse.body().isEmpty()) return;
                ...
                apiService.getCompletedTeamProgressRaw(
                        "eq." + challengeId,
                        "eq.true",
                        "completed_at.asc"
                ).enqueue(new Callback<List<JsonObject>>() {
                        ...
                        apiService.updateTeamChallengeProgressRaw("eq." + progressId, payload)
                                .enqueue(new Callback<Void>() {
                                    ...
                                    public void onResponse(Call<Void> call, Response<Void> r) {
                                        if (r.isSuccessful() && rewardListener != null) {
    ...
    public void deleteTeam(int teamId, int userId, DataCallback<Void> callback) {
        // 1. Сначала удаляем с сервера
        apiService.deleteTeamRaw("eq." + teamId).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    // 2. Очищаем локальный кэш этой конкретной команды
                    executor.execute(() -> {
                        try {
